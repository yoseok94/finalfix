<template>
    ...
    <tr v-for="(plan, index) in filteredPlans" :key="index">
        ...
        <td>{{ plan.details }}</td>
        <td>{{ plan.remarks }}</td>
        <td>{{ plan.status === 'processing' ? '처리중' : '완료' }}</td>
        <td>
            <button @click="approve(plan.id)" :disabled="plan.status === 'completed'">승인요청</button>
            <button @click="cancelApproval(plan.id)" :disabled="plan.status === 'processing'">승인취소</button>
        </td>
    </tr>
</tbody>
                </table>
            ...
            <div class="common-buttons">
                <router-link to="/businessDisbursementAdd">
                    <button class="btn-add">등록</button>
                </router-link>
                <router-link to="/businessDisbursementEdit">
