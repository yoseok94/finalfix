<template>
      <div>
  <br>
     <h1 style="text-align: center;">공제 리스트 작성 페이지</h1>
     <br>
   ...
   <form @submit.prevent="saveData">
    <table class="salary-table">
        ...
        <tbody>
        <tr v-for="deductionItem in displayedItems" :key="deductionItem.itemCode">
            <td><input class="table-input" type="text" v-model="deductionItem.itemCode" /></td>
                <td><input class="table-input" type="text" v-model="deductionItem.itemName" /></td>
                <td><input class="table-input" type="text" v-model="deductionItem.displayOrder" /></td>
                <td><input class="table-input" type="text" v-model="deductionItem.calculation" /></td>
                <td><input class="table-input" type="text" v-model="deductionItem.calculationMethod" /></td>
  ...
  data() {
    ...
    return {
      ...
      items: Array.from({ length: 50 }, (_, i) => ({
        itemCode: `EXM-${Math.floor(Math.random() * 1000)}`,
        itemName: `Deduction Item ${i}`,
        displayOrder: i + 1,
